...
CUB3D_PATH = "../cub3d_bonus"  # change to 'cub3d_bonus' here to test the other bin
...
def test_multiple_commas1():
	stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "multiple_commas1.cub")
	valgrind_issue = parse_valgrind_output(stderr_printout)
	assert exit_status > 0
	assert valgrind_issue == OK
	assert confirm_runtime_error_reported(stderr_printout) == OK

def test_multiple_commas2():
	stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "multiple_commas2.cub")
	valgrind_issue = parse_valgrind_output(stderr_printout)
	assert exit_status > 0
	assert valgrind_issue == OK
	assert confirm_runtime_error_reported(stderr_printout) == OK

def test_multiple_commas3():
	stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "multiple_commas3.cub")
	valgrind_issue = parse_valgrind_output(stderr_printout)
	assert exit_status > 0
	assert valgrind_issue == OK
	assert confirm_runtime_error_reported(stderr_printout) == OK

def test_multiple_commas4():
	stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "multiple_commas4.cub")
	valgrind_issue = parse_valgrind_output(stderr_printout)
	assert exit_status > 0
	assert valgrind_issue == OK
	assert confirm_runtime_error_reported(stderr_printout) == OK

def test_multiple_commas5():
	stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "multiple_commas5.cub")
	valgrind_issue = parse_valgrind_output(stderr_printout)
	assert exit_status > 0
	assert valgrind_issue == OK
	assert confirm_runtime_error_reported(stderr_printout) == OK

def test_no_val_between_commas():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "no_val_between_commas.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK

def test_no_val_between_commas2():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "no_val_between_commas2.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK

def test_space_between_commas():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "space_between_commas.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK

def test_tab_between_commas():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "tab_between_commas.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK

def test_alphabetic_between_commas():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "alphabetic_between_commas.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK

def test_missing_ceiling():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "missing_ceiling.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK

def test_two_ceilings():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "two_ceilings.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK
...
def false_order():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "false_order.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK

def test_two_north_textures():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "two_north_textures.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK

def test_unexisting_texture_file():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "unexisting_texture_file.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK

def test_texture_file_is_directory():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "texture_file_is_directory.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK

def test_no_permission_for_texture():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "no_permission_for_texture.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK

def test_texture_not_square():
    stderr_printout, exit_status = run_valgrind_with_binary(TEST_FILES_BAD + "texture_not_square.cub")
    valgrind_issue = parse_valgrind_output(stderr_printout)
    assert exit_status > 0
    assert valgrind_issue == OK
    assert confirm_runtime_error_reported(stderr_printout) == OK
